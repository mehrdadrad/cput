...
impl Stats {
    ...
    pub fn start(&self) {
        ...
        let tcp_server_addr = String::from(&self.tcp_server_addr);
        ...
        if !tcp_server_addr.is_empty() {
            thread::spawn(|| Self::tcp_server(tcp_server_addr, tcp_tx, total_rx_cloned));
        }
        ...
        loop {
            select! {
                recv(self.rx) -> msg => {
                    match msg {
                        Ok(x) => {
                           if x.tx == 0 && x.rx == 0 {
                              if !self.tcp_client_addr.is_empty() {
            ...
            if timer.ticker() && !self.tcp_client_addr.is_empty() {
                let tb = Self::tcp_client(
                    &self.tcp_client_addr,
                    TcpBucket {
